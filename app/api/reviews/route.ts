...
export const GET = withApi(async (req: NextRequest) => {
  ...
  return successJson(await reviewService.list(workspaceId));
});
...
export const POST = withApi(async (req: NextRequest) => {
  ...
  const review = await reviewService.create(workspaceId, body);
  return successJson(review, { status: 201, message: "Review created" });
});
